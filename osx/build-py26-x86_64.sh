...
rm -Rf $TMP_DIR
$BIN_DIR/pip2.6 install --build $TMP_DIR --no-use-wheel --pre coverage

COVERAGE_VERSION=$($BIN_DIR/pip2.6 show coverage | grep Version | grep -v Metadata-Version | sed 's/Version: //')
...
cd $OUT_DIR
zip -r ../coverage-${COVERAGE_VERSION}_py26_osx-x64.zip *
