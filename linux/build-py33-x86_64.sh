...
rm -Rf $TMP_DIR
$BIN_DIR/pip3.3 install --build $TMP_DIR --pre coverage

COVERAGE_VERSION=$($BIN_DIR/pip3.3 show coverage | grep Version | grep -v Metadata | sed 's/Version: //')
...
cd $OUT_DIR
tar cvzpf ../coverage-${COVERAGE_VERSION}_py33_linux-x64.tar.gz *
