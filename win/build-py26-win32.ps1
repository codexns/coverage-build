...
c:\Python26-x86\Scripts\pip.exe uninstall -y coverage
c:\Python26-x86\Scripts\pip.exe install --build "$tmpDir" --no-use-wheel --pre coverage

$coverageVersion = ""
c:\Python26-x86\Scripts\pip.exe show coverage | foreach-object {
    $splitLine = $_.split(": ")
...
copy-item -recurse C:\Python26-x86\Lib\site-packages\coverage $outDir\

&"${env:ProgramFiles}\7-Zip\7z.exe" a -r -tzip $outDir\..\coverage-${coverageVersion}_py26_windows-x32.zip $outDir\*
