...
c:\Python33\Scripts\pip.exe uninstall -y coverage
c:\Python33\Scripts\pip.exe install --build "$tmpDir" --no-use-wheel --pre coverage


$coverageVersion = ""
c:\Python33\Scripts\pip.exe show coverage | foreach-object {
    $splitLine = $_.split(": ")
...
copy-item -recurse C:\Python33\Lib\site-packages\coverage $outDir\

&"${env:ProgramFiles}\7-Zip\7z.exe" a -r -tzip $outDir\..\coverage-${coverageVersion}_py33_windows-x64.zip $outDir\*